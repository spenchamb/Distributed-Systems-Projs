...
def main():
  ...
  procs = [] #holds all active processes
  for i in range(filesystem['num_replicas']):
    p1 = subprocess.Popen(f'python3 replica.py {i}', shell=True)
    procs.append(p1)

  time.sleep(2) #delay to allow replicas to get settled. only necessary because implementation on a single system

  for i in range(filesystem['num_clients']):
    procs.append(subprocess.Popen(f'python3 client.py {i}', shell=True))
  
  while len(procs) > 0: #while there are active processes, stay alive
    for p in procs:
